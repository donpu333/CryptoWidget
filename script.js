...
const API_CONFIG = {
    ...
    WARNING_THRESHOLD: 0.05 // 5% предупреждение до цены алерта
};
...
let currentPrices = {};
let warningAlertsSent = {}; // Для отслеживания отправленных предупреждений
...
// Функция для проверки предупреждений (за 5-10% до цены алерта)
async function checkWarningAlerts() {
    ...
    for (const alert of activeAlerts) {
        try {
            ...
            const warningKey = `warning_${alert.id}`;
            const lastWarning = warningAlertsSent[warningKey] || 0;

            // Проверяем, находится ли текущая цена в пределах 5-10% от целевой
            let warningMessage = '';
            let warningPercent = 0;

            if (alert.condition === '>') {
                // Для лонг алертов (цена должна вырасти)
                const distanceToTarget = targetPrice - price;
                const percentToTarget = (distanceToTarget / targetPrice) * 100;
                
                if (percentToTarget <= 10 && percentToTarget > 5) {
                    warningPercent = 10;
                    warningMessage = `⚠️ Приближается лонг алерт! ${alert.symbol} в ${10-percentToTarget.toFixed(1)}% от цели ${targetPrice}. Текущая цена: ${price}`;
                } else if (percentToTarget <= 5 && percentToTarget > 2) {
                    warningPercent = 5;
                    warningMessage = `🚨 Близко к лонг алерту! ${alert.symbol} в ${5-percentToTarget.toFixed(1)}% от цели ${targetPrice}. Текущая цена: ${price}`;
                } else if (percentToTarget <= 2) {
                    warningPercent = 2;
                    warningMessage = `🔴 ОЧЕНЬ БЛИЗКО! ${alert.symbol} менее чем в 2% от цели ${targetPrice}. Текущая цена: ${price}`;
                }
            } else if (alert.condition === '<') {
                // Для шорт алертов (цена должна упасть)
                const distanceToTarget = price - targetPrice;
                const percentToTarget = (distanceToTarget / targetPrice) * 100;
                
                if (percentToTarget <= 10 && percentToTarget > 5) {
                    warningPercent = 10;
                    warningMessage = `⚠️ Приближается шорт алерт! ${alert.symbol} в ${10-percentToTarget.toFixed(1)}% от цели ${targetPrice}. Текущая цена: ${price}`;
                } else if (percentToTarget <= 5 && percentToTarget > 2) {
                    warningPercent = 5;
                    warningMessage = `🚨 Близко к шорт алерту! ${alert.symbol} в ${5-percentToTarget.toFixed(1)}% от цели ${targetPrice}. Текущая цена: ${price}`;
                } else if (percentToTarget <= 2) {
                    warningPercent = 2;
                    warningMessage = `🔴 ОЧЕНЬ БЛИЗКО! ${alert.symbol} менее чем в 2% от цели ${targetPrice}. Текущая цена: ${price}`;
                }
            ...
            // Отправляем предупреждение если нашли подходящее и прошло больше 5 минут с последнего
            if (warningMessage && (now - lastWarning > 300000)) {
                // Отправка в Telegram
                if (alert.notificationMethods.includes('telegram') && alert.chatId) {
                    try {
                        await sendTelegramNotification(warningMessage, alert.chatId);
                    } catch (error) {
                ...
                showNotification('Предупреждение алерта', warningMessage);
                
                // Обновляем время последнего предупреждения
                warningAlertsSent[warningKey] = now;
            }
        } catch (error) {
            console.error(`Error checking warning for alert ${alert.id}:`, error);
        }
    }
}

// Остальной код остается без изменений (функции из предыдущего ответа)
// [Здесь должен быть весь остальной код из предыдущего ответа]
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Запускаем проверку предупреждений каждые 30 секунд
        setInterval(checkWarningAlerts, 30000);
