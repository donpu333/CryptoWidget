...
async function addUserAlert(symbol, type, condition, value, notificationMethods, notificationCount, chatId) {
    try {
        ...
        loadUserAlerts(currentAlertFilter);
        // Подсвечиваем новый алерт
        highlightNewAlert(newAlert.id);
        ...
        alertForm.addEventListener('submit', async function(e) {
            ...
            if (editAlertId) {
                ...
                if (success) {
                    showNotification('Успешно', `Алерт для ${symbol} создан`);
                    resetForm();
                    // Обновление списка уже происходит внутри addUserAlert()
                }
            }
        });
    }
...
// Функция для подсветки нового алерта
function highlightNewAlert(alertId) {
    const alertElement = document.getElementById(`alert-${alertId}`);
    if (alertElement) {
        alertElement.classList.add('new-alert-highlight');
        setTimeout(() => {
            alertElement.classList.remove('new-alert-highlight');
        }, 2000);
    }
}
